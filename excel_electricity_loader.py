...
class ExcelElectricityDataLoader:
    ...
    def _read_excel_file(self, file_path):
        ...
        try:
            ...
            if 'timestamp' in df.columns:
                ...
                print(f"  Final timestamp status: {valid_count} valid timestamps out of {len(df)} rows")
                ...
                if timestamp_candidates:
                    ...
                    print(f"  No suitable timestamp column found in {file_path}")
                    return None
                ...
                if os.path.exists(file_path):
                    ...
                    if df is not None:
                        ...
                        if price_col:
                            ...
                            print(f"  Warning: Could not identify price column in {file_path}")
                    
                    # Break once we've found and processed a file
                    break
            
            if not file_found:
                print(f"Warning: No electricity price Excel file found for {year}")
    ...
    def load_solar_wind_forecasts(self, years=None):
            ...
            for filename in possible_files:
                ...
                if os.path.exists(file_path):
                    ...
                    if df is None or 'timestamp' not in df.columns:
                        print(f"  Skipping file {file_path}, no valid timestamp column")
                        continue
                    ...
                    if solar_col is not None:
                        ...
                        solar_data = solar_data.rename(columns={solar_col: 'solar_forecast'})
                        
                        # Check for 15-minute intervals and resample if needed
                        if self._is_15min_data(solar_data):
                            print(f"  Detected 15-minute interval solar data, resampling to hourly")
                            solar_data = self._resample_to_hourly(solar_data, value_columns=['solar_forecast'])
                        
                        solar_dfs.append(solar_data)
                    
                    # Process wind data
                    if wind_onshore_col is not None or wind_offshore_col is not None:
                        print(f"  Found wind forecast columns: Onshore={wind_onshore_col}, Offshore={wind_offshore_col}")
                        ...
                        if wind_onshore_col is not None and wind_offshore_col is not None:
                            ...
                            wind_data['wind_forecast'] = df[wind_onshore_col].add(df[wind_offshore_col], fill_value=0)
                        ...
                        if self._is_15min_data(wind_data):
                            print(f"  Detected 15-minute interval wind data, resampling to hourly")
                            wind_data = self._resample_to_hourly(wind_data, value_columns=['wind_forecast'])
        ...
        # Combine all years
        solar_df = pd.concat(solar_dfs, ignore_index=True) if len(solar_dfs) > 0 else None
        wind_df = pd.concat(wind_dfs, ignore_index=True) if len(wind_dfs) > 0 else None
        
        # Add debugging info
        if solar_df is not None:
            print(f"Solar forecast data: {len(solar_df)} rows, range: {solar_df['timestamp'].min()} to {solar_df['timestamp'].max()}")
        else:
            print("No solar forecast data found!")
        
        if wind_df is not None:
            print(f"Wind forecast data: {len(wind_df)} rows, range: {wind_df['timestamp'].min()} to {wind_df['timestamp'].max()}")
        else:
    ...
    def load_power_system_features(self, years=None):
        ...
        load_dfs = []
        for year in years:
            file_path = os.path.join(self.data_dir, f"load_forecast_{year}.xlsx")
            if os.path.exists(file_path):
                ...
                if forecast_col:
                    ...
                    if len(df) > 0:
                        print(f"  Sample values: {df[forecast_col].head(3).tolist()}")
                    ...
                    if df[forecast_col].isna().all():
                        print(f"  WARNING: All values in '{forecast_col}' became NaN after conversion")
                    ...
                    if self._is_15min_data(df):
                        print(f"  Detected 15-minute interval load data, resampling to hourly")
                        df = self._resample_to_hourly(df, value_columns=["load_forecast"])
                    
                    df["feature_type"] = "load_forecast"
                    df = df[["timestamp", "load_forecast", "feature_type"]]
                    load_dfs.append(df)
                else:
                    print(f"  Warning: Could not identify load forecast column in {file_path}")
        ...
        gen_dfs = []
        for year in years:
            file_path = os.path.join(self.data_dir, f"total_generation_{year}.xlsx")
            if os.path.exists(file_path):
                ...
                if gen_col:
                    ...
                    if self._is_15min_data(df):
                        print(f"  Detected 15-minute interval data, resampling to hourly")
                        df = self._resample_to_hourly(df, value_columns=["total_generation"])
                    
                    df["feature_type"] = "total_generation"
                    gen_dfs.append(df)
                else:
                    print(f"  Warning: Could not identify generation column in {file_path}")
    ...
    def load_import_export(self, years=None):
        ...
        for year in years:
            ...
            for file in files:
                ...
                if import_col is not None or export_col is not None:
                    ...
                    if self._is_15min_data(df):
                        print(f"  Detected 15-minute interval data, resampling to hourly")
                        df = self._resample_to_hourly(df, value_columns=['net_flow'])
        ...
        agg_import_export = import_export_df.groupby('timestamp')['net_flow'].sum(min_count=1).reset_index()
        agg_import_export.rename(columns={'net_flow': 'net_import_export'}, inplace=True)
    ...
    def load_fuel_prices(self, years=None):
        ...
        for year in years:
            ...
            if os.path.exists(file_path):
                ...
                if price_col:
                    ...
                    print(f"  Warning: Could not identify oil price column in {file_path}")

        # Load natural gas prices
        gas_dfs = []
        for year in years:
            file_path = os.path.join(self.data_dir, f"natural_gas_price_{year}.xlsx")
            if os.path.exists(file_path):
                ...
                if price_col:
                    print(f"  Using column '{price_col}' for natural gas price")
                    df = self._safe_convert_to_numeric(df, price_col)
                    df = df.rename(columns={price_col: 'natural_gas'})
                    df = df[['timestamp', 'natural_gas']]
                    
                    # Debug
                    print(f"  Natural gas price data: {len(df)} rows, range: {df['timestamp'].min()} to {df['timestamp'].max()}")
                    if len(df) > 0:
                        print(f"  Sample values: {df['natural_gas'].head(3).tolist()}")
                    
                    gas_dfs.append(df)
                else:
                    print(f"  Warning: Could not identify gas price column in {file_path}")
        ...
        fuel_prices_daily = pd.merge(all_prices[0], all_prices[1], on='timestamp', how='outer') if len(all_prices) > 1 else all_prices[0]
        ...
        for col in fuel_prices_daily.columns:
            if col != 'timestamp':
                if not pd.api.types.is_numeric_dtype(fuel_prices_daily[col]):
                    ...
                    fuel_prices_daily[col] = pd.to_numeric(fuel_prices_daily[col], errors='coerce')
        ...
        if years and len(years) == 1:
            ...
            start_date = min(start_date, min_date) if start_date is not None else min_date
            end_date = max(end_date, max_date) if end_date is not None else max_date
            
            print(f"  Extended date range to cover full year: {start_date} to {end_date}")
        ...
        for col in merged_df.columns:
            if col != 'timestamp':
                ...
                print(f"    {col}: {merged_df[col].notna().sum()} non-null values, {missing} missing values")
        ...
        if 'oil' in merged_df.columns and 'natural_gas' in merged_df.columns:
            print(f"  Final fuel price dataset: {len(merged_df)} rows with both oil and gas prices")
        elif 'oil' in merged_df.columns:
            print(f"  Final fuel price dataset: {len(merged_df)} rows with only oil prices")
        elif 'natural_gas' in merged_df.columns:
            print(f"  Final fuel price dataset: {len(merged_df)} rows with only natural gas prices")
        else:
    ...
    def prepare_full_dataset(self, years=None):
        ...
        if import_export_df is not None:
            try:
                full_df = pd.merge(full_df, import_export_df, on='timestamp', how='left')
            except Exception as e:
        ...
        if fuel_prices_df is not None:
            try:
                full_df = pd.merge(full_df, fuel_prices_df, on='timestamp', how='left')
            except Exception as e:
        ...
        for col in full_df.columns:
            ...
            percent_missing = (missing / len(full_df)) * 100 if len(full_df) > 0 else 0
        ...
        if len(full_df) > 0:
            for col in numeric_cols:
                if col not in essential_columns:
                    ...
                    print(f"  {col}: {missing_before-missing_after} missing values filled")
        ...
        for feature in expected_features:
            if feature in full_df.columns:
                ...
                percent = (non_null / len(full_df)) * 100 if len(full_df) > 0 else 0
